...
#include <stdio.h>
	...
	int csrIndex(uint16_t endereco) {
		switch (endereco) {
			...
			case 0x343: return 5; // mtval
			case 0x344: return 6; // mip
			default: return -1;   // CSR não suportado
		}
	}
    ...
    *mstatus_ptr = mstatus;
	...
	void registrarExcecao(uint32_t causa, uint32_t endereco_instrucao, uint32_t tval, uint32_t* registradoresCSRs, FILE *output, uint32_t *pc_ptr){
		...
		const char* nome_exc =
			(causa == 0x0) ? "instruction_misaligned" :
			(causa == 0x1) ? "instruction_fault" :
	...
	{ // argumento para abrir o projeto no terminal, entrega a entrada e fala a saida
		// "./meuprograma" "entrada.hex"  "saida.out"
		 ...
		 FILE *qemu_terminal_in = fopen(argv[3], "r"); //Abre o arquivo de entrada UART
		 FILE *qemu_terminal_out = fopen(argv[4], "w"); //Abre/cria o arquivo de saída UART
    ...
    if (funct3 == 0b000) {
        ...
        if (endereco >= 0x10000000 && endereco <= 0x10000007) {
            if (endereco == 0x10000000) {
                ...
                registradoresUART[5] = feof(qemu_terminal_in) ? 0x00 : 0x01;
		...
		case 0b0010111:
			...
			if (imm_csr == 0x300) { // mstatus: tratar só MIE e MPIE
